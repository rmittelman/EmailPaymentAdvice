...
namespace Email_Payment_Advice
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            else
            {
                ...
                foreach (var school in Schools)
                {
                    ...
                    LogIt.LogInfo($"Processing: School={school}, From={args[0]}, To={args[1]}");
                    DataSet ds = new DataSet();
                    try
                    {
                        LogIt.LogDebug("Getting payments made.");
                        ...
                        new KeyValuePair<string,string>("_fromDate", DateTime.Parse(args[0]).ToString("yyyy-MM-dd")),
                        new KeyValuePair<string,string>("_toDate", DateTime.Parse(args[1]).ToString("yyyy-MM-dd"))
                    };
                    ...
                    {
                        LogIt.LogInfo($"No payments to process for {school} between {args[0]} and {args[1]}");
                        Environment.Exit(1);
                    }
                        ...
                        if ((int)row[idCol] != curStudentID)
                        {
                            ...
                            if (curStudentID != 0)
                            {
                                ...
                                try
                                {
                                    ...
                                    Execute(curStudentFirst, curStudentLast, curStudentEmail, curSchoolContact, curSchoolContactEmail, textBody, htmlBody).Wait();
            ...
            using (ODBCClass o = new ODBCClass("MySql"))
            {
                using (OdbcCommand oCommand = o.GetCommand(update_sql))
                {
                    ...
                    try
                    {
                        ...
                        LogIt.LogDebug($"Updated student ID {student}, Rows affected: {rows}");
                    }
                    catch (Exception ex)
                    {
        ...
        private static void update_payments(List<int> paymentList, DateTime now)
        {
            ...
            using (ODBCClass o = new ODBCClass("MySql"))
            {
                using (OdbcCommand oCommand = o.GetCommand(update_sql))
                {
                    ...
                    {
                        var rows = oCommand.ExecuteNonQuery();
                        LogIt.LogDebug($"Updated payment IDs {inList}, Rows affected: {rows}");
                    }
                    catch (Exception ex)
                    {
        ...
        static async Task Execute(string studFirst, string studLast, string emailAddress, string schoolContact, string schoolEmail, string txtBody, string htmBody)
        {
            try
            {
